...
const WILDCARD_PATTERNS: { pattern: RegExp; label: string }[] = [
  { pattern: /\(\*:\*\)/, label: "Wildcard tool permission (Tool(*:*))" },
  {
    pattern: /"allow"\s*:\s*\[\s*"\*"\s*\]/,
    label: 'Wildcard allow list ("allow": ["*"])',
  },
...
export const noWildcardPermissionsRule: Rule = {
  ...
  severity: "error",
  description:
    "Tool permissions must not use wildcards — be explicit about allowed tools",
