...
describe('lint', () => {
  it('returns a report with zero results for empty rules', () => {
    const report = lint('/test.md', '## Project Overview\n\n## Constraints\n\n## Acceptance Criteria', []);
  ...
  it('counts errors and warnings separately', () => {
    const errorRule = {
      ...
      severity: 'error' as const,
      description: 'test',
      check: () => [{ ruleId: 'T001', severity: 'error' as const, message: 'err' }],
    };
    const warningRule = {
      id: 'T002',
      name: 'warning-rule',
      ...
      description: 'test',
      check: () => [{ ruleId: 'T002', severity: 'warning' as const, message: 'warn' }],
    };
