...
describe('parseSpecFile', () => {
  it('parses sections from markdown headings', () => {
    ...
    expect(result.sections[0]).toMatchObject({ heading: 'Title', level: 1, line: 1 });
    expect(result.sections[1]).toMatchObject({ heading: 'Overview', level: 2, line: 5 });
    expect(result.sections[2]).toMatchObject({ heading: 'Constraints', level: 2, line: 9 });
  });
